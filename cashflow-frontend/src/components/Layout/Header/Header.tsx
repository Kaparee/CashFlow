...
const Header: React.FC = () => {
    ...
    return (
        <div className={`row align-items-center text-center  ${!isHomePage ? 'shadow py-2' : 'px-1 pt-2 p-sm-3'} ${windowWidth > 768 && isHomePage ? 'fixed-top' : ''} transition ${isHomePage &&isScrolled && (windowWidth > 768) ? `${s.scrolledHeader} mx-3 mt-2 shadow rounded-4` : 'bg-white'}`}>
            <div className={`${isHomePage ? 'col-6 col-md-3 order-1 d-flex justify-content-start justify-content-md-center align-items-center' : 'col-12 text-center'}`}>
